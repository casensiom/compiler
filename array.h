...
#define AC_ARRAY_SET(TYPE, VALUE, COUNT) (TYPE##Array){.items = (TYPE *)(VALUE), .count = (COUNT), .capacity = 0}
...
// VAR is a pointer of TYPE to iterate all elements in INSTANCE
// To get the position do VAR - INSTANCE.count
#define AC_ARRAY_FOREACH(TYPE, VAR, INSTANCE) for (TYPE *VAR = (INSTANCE).items; VAR < (INSTANCE).items + (INSTANCE).count; ++VAR)
