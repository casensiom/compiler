...
void report_error(TokenLocation loc, const char *pos, const char *msg, ...) {
  const char *line_start = NULL;
  const char *line_end = NULL;
  get_line_at(loc.file->content, pos, &line_start, &line_end);
  int line_len = line_end - line_start;

  // LOG_DEBUG("POINTER: (%p) [%p <= %p <= %p] %.*s", loc.file->content,
  // line_start, pos, line_end, line_len, line_start);
  // LOG_DEBUG("MESSAGE: %s", message); LOG_DEBUG("LOCATION: line = %lu, column
  // = %lu", loc.line, loc.column);
